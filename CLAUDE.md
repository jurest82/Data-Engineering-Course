...
- **Raíz**: devcontainer para trabajo a nivel de monorepo (ej. generar los diagramas de arquitectura). `.docker/Dockerfile` es multi-stage: `img-base` (Python 3.13 + Node 24 + herramientas comunes, incluye `~/.cfnlintrc` para que `cfn-lint` ignore el falso positivo W3005 que agrega Serverless Framework) → `img-backend` (suma `boto3`, solo para que el linter/autocompletado local resuelva los imports; el runtime de Lambda ya lo trae, nunca se empaqueta) / `img-infrastructure` (sin diferencias adicionales por ahora) / `img-root` (suma `graphviz` + la librería Python `diagrams`, usada para generar los diagramas de arquitectura en `docs/` — uno por pipeline, ver `docs/architecture.py` — no es específico de `backend` ni `infrastructure`, por eso vive en la imagen raíz).
...
- **`docs/`**: diagramas de arquitectura, uno por pipeline (`architecture_batch.png`, `architecture_streaming.png`), generados con la librería Python `diagrams` desde `docs/architecture.py` (correrlo con `python3 architecture.py` dentro del devcontainer de la raíz, que trae `graphviz` vía `img-root`). Deliberadamente dos diagramas separados y no uno solo: un único diagrama tiene que dibujar las aristas transversales de ambos pipelines hacia Mongo/Secrets Manager y hacia las alarmas compartidas, y esas aristas fuerzan curvas largas en el auto-layout de Graphviz; separados, cada pipeline es una cadena recta.
- **`frontend/`** y **`etl/`**: no existen todavía, no son parte del alcance actual (ver "Pendientes abiertos"); ya no se mencionan en `README.md` como si fueran parte del monorepo. Mientras no exista frontend, el flujo batch se prueba directo contra el API (ej. con Postman).
...
- Stack `infrastructure/serverless/iot` — ✅ desplegado: una **Policy de IoT genérica y reusable** (no ligada a un dispositivo específico) que usa variables de política de IoT (`${iot:Connection.Thing.ThingName}`) para que cada certificado solo pueda conectarse/publicar en su propio tópico (`sensors/traffic/<su-thing-name>/data`), y un `AWS::IoT::TopicRule` (`SELECT *, topic(3) AS thing_name FROM 'sensors/traffic/+/data'`) que enruta cada mensaje a `SensorReadingsQueue`, con su propio rol IAM (`sqs:SendMessage` acotado a esa cola). El `topic(3) AS thing_name` agrega al mensaje el segmento del tópico correspondiente al nombre del Thing (no se reusa el nombre `sensor_id` porque la documentación de AWS IoT SQL no especifica cómo resolvería una colisión de nombre con `SELECT *`): como la Policy ya garantiza que ese segmento coincide con el `ThingName` del certificado que publicó (`iot:Publish` acotado a `sensors/traffic/${iot:Connection.Thing.ThingName}/data`), es una fuente de identidad confiable, a diferencia del campo `sensor_id` del JSON, que el propio dispositivo controla y podría falsear — `PersistSensorReading` usa `thing_name` para sobrescribir `sensor_id` antes de validar/persistir (ver más abajo). Deliberadamente **no** incluye el Thing/certificado de un dispositivo concreto — eso es aprovisionamiento por sensor, no un recurso fijo del stack (ver `provision_sensor.sh` más abajo).
- Stack `backend/serverless/streaming` — ✅ desplegado: Lambda `PersistSensorReading` (batchSize 1, mismo patrón de defensa en profundidad + auto-envío a su DLQ que Lambda 3 del batch) + su rol IAM. Esquema de la lectura: `sensor_id`, `city`, `road`, `speed_avg`, `vehicle_count`, `recorded_at` — sin PII (no hay persona involucrada), así que no usa cifrado ni la capa `Security`. `sensor_id` que llega en el JSON del sensor se descarta siempre: la Lambda lo sobrescribe con `thing_name` (inyectado por el `TopicRule`, ver arriba) antes de validar, para que el valor persistido/usado en la DLQ sea el que la Policy verificó, no el que el dispositivo afirme. Persiste en MongoDB Atlas, misma base `trafficMonitoring`, colección nueva `trafficSensorReadings`.
