...
## RDS PostgreSQL instance (future ETL target)

Stack `infrastructure/serverless/rds` — ✅ built: a single, publicly accessible PostgreSQL instance (`db.t4g.micro`, 20GB `gp2`, no Multi-AZ), meant as the destination for a future ETL out of MongoDB Atlas. **Only the instance itself is built so far** — its schema, migrations, and the ETL pipeline that would actually populate it are not built yet (see "Open items").
...
- **Master credentials only**, autogenerated by CloudFormation (`GenerateSecretString` + `AWS::SecretsManager::SecretTargetAttachment`, which also populates `host`/`port`/`dbname` into the same secret once the instance exists) — deliberately no separate app/read-only database user, unlike Juan Pablo's own production data warehouse project; this is a course example, not something needing least-privilege database roles.
- **No read replica, RDS Proxy, Enhanced Monitoring, Performance Insights, custom parameter group, or secrets rotation** — all complexity from Juan Pablo's own reference projects that doesn't apply to this course's scope or free-tier budget.
...
- ETL from MongoDB Atlas into the `rds` PostgreSQL instance: the instance itself is deployed (see "RDS PostgreSQL instance"), but its schema/migrations and the actual pipeline that would populate it are not built yet.
