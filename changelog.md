...
- `infrastructure` subproject:
  - Serverless Framework stacks for shared base resources:
    ...
    - IoT Core policy and topic rule that route sensor readings to the streaming queue (`iot`)
...
- `backend` subproject:
  ...
  - Streaming sensor readings pipeline:
    - `PersistSensorReading` Lambda: re-validates each sensor reading from SQS and saves it to MongoDB Atlas, or forwards invalid readings to the dead-letter queue itself
