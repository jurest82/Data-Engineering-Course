...
7. Deploy `serverless/etl` stack: SQS queues and dead-letter queues for each stage (Extractor, Dispatcher, Transformer) of the ETL's two domain pipelines (`accidentReports`, `sensorReadings`), and an SNS topic per domain that the Transformer's queue subscribes to; independent from every other stack above
