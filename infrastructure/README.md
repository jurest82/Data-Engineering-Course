...
5. Deploy `serverless/rds` stack (a single-instance, publicly accessible PostgreSQL RDS instance for the ETL that copies data out of MongoDB Atlas -- uses the account's pre-existing default VPC instead of a dedicated one, so `DEFAULT_VPC_ID`/`DEFAULT_VPC_SUBNET_IDS` must point to it; master credentials are auto-generated by CloudFormation straight into Secrets Manager, unlike the MongoDB Atlas credentials secret above which has to be supplied manually; independent from every other stack above)
6. Deploy `serverless/etl` stack: SQS queues and dead-letter queues for each stage (Extractor, Dispatcher, Transformer) of the ETL's two domain pipelines (`accidentReports`, `sensorReadings`), and an SNS topic per domain that the Transformer's queue subscribes to; independent from every other stack above, but must be deployed **before** `backend`'s `batch` and `streaming` stacks (they publish to this stack's SNS topics right after persisting to MongoDB Atlas, see `backend/README.md`)
7. Deploy `serverless/alerts` stack **last**, only after `backend`'s `batch` stack is already deployed (see `backend/README.md`): SNS topic with an email subscription, an account-wide AWS Budget on cost, and CloudWatch alarms — on both pipelines' dead-letter queues (only need `serverless/queue`) and on `SplitAndEnqueue` Lambda errors (needs `backend`'s `batch` stack) — the only stack in this repo that depends on a `backend` stack already being deployed, rather than the other way around
